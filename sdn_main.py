...
def generate_data(config):
    loader_n_train = DataLoader(
        dataset=SDN(mode='Train', num_segments=config.Train.num_segments, is_normal=True),
    ...
    loader_a_train = DataLoader(
        dataset=SDN(mode='Train', num_segments=config.Train.num_segments, is_normal=False),
        batch_size=config.Train.batch_size,
        shuffle=True,
        num_workers=config.Train.num_workers,
        drop_last=True,
...
if __name__ == "__main__":
    ...
    best_ckpt = os.path.join(config.Model.ckpt_path, "sdn_best.pth")
    tot_train_time = 0.0
    tot_test_time = 0.0
    for epoch in range(epochs):
        ...
        logger.info(f'epoch: {epoch+1}|{epochs}; loss: {loss:.5f}; total auc: {auc:.5f}; anomaly auc: {ano_auc:.5f}; _far: {_far:.5f}')
