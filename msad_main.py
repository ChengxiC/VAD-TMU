...
def generate_data(config):
    loader_n_train = DataLoader(
        dataset=MSAD(mode='Train', num_segments=config.Train.num_segments, is_normal=True),
    ...
    loader_a_train = DataLoader(
        dataset=MSAD(mode='Train', num_segments=config.Train.num_segments, is_normal=False),
        batch_size=config.Train.batch_size,
        shuffle=True,
        num_workers=config.Train.num_workers,
        drop_last=True,
    ...
    loader_test = DataLoader(
        dataset=MSAD(mode='Test', num_segments=config.Test.num_segments),
...
if __name__ == "__main__":
    ...
    config = make_config('./config_yaml/config_msad.yaml')
    ...
    best_ckpt = os.path.join(config.Model.ckpt_path, "msad_best.pth")
    tot_train_time = 0.0
    tot_test_time = 0.0
    for epoch in range(epochs):
        ...
        t2 = time.time()
        auc, ano_auc, _far = test(model, test_loader, device, gt_path)
        t3 = time.time()
        ...
        logger.info(f'epoch: {epoch+1}|{epochs}; loss: {loss:.5f}; total auc: {auc:.5f}; anomaly auc: {ano_auc:.5f}; false alarm rate: {_far}')

    avg_test_time = tot_test_time / epochs
    logger.info(f'*******************************best auc: {best_auc:.5f}; anomaly auc: {_ano_auc: .5f}***********************************')
